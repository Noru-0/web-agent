...
class ExplorationLoop:
    ...
    def __init__(
        ...
        # DOM cache: Prevents duplicate LLM calls for same visual state.
        # Key: DOM+visual hash, Value: (semantic_summary, screen_type, action_semantics)
    ...
    async def _capture_screen(self) -> Screen:
        ...
        # Calculate cache key for combined DOM + screenshot state.
        dom_hash = hashlib.md5(html.encode('utf-8')).hexdigest()
        screenshot_hash = hashlib.md5((screenshot or "").encode('utf-8')).hexdigest()
        cache_key = f"{dom_hash}:{screenshot_hash}"
        ...
        if cache_key in self.dom_cache:
            logger.info(f"[CACHE HIT] Reusing LLM analysis for key: {cache_key[:16]}...")
            semantic_summary, screen_type, action_semantics = self.dom_cache[cache_key]
        else:
            logger.info(f"[CACHE MISS] Calling LLM for new key: {cache_key[:16]}...")
            ...
            # Cache the result
            self.dom_cache[cache_key] = (semantic_summary, screen_type, action_semantics)
            logger.debug(f"Cached LLM result for key: {cache_key[:16]}...")
...
class SimpleExplorationAdapter(ExplorationAdapter):
    ...
    async def execute_action(self, action_semantic: ActionSemantic, env) -> bool:
        ...
        hints = getattr(action_semantic, "grounding_hints", None)
        if hints is None:
            hints = getattr(action_semantic, "hints", None)
