...
def run_task_synthesis(explorer_name: str, output_base_dir: Path = None):
    """
    ...
    try:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("Running Task Synthesis (Phase 3)")
        logger.info("=" * 60)
        ...
        logger.info(f"Loaded {len(screens)} screens with actions")
...
async def run_exploration_cli(args):
    ...
    try:
        ...
        # Automatically run task synthesis (Phase 3) unless disabled
        if not args.skip_task_synthesis:
            logger.info("\nAutomatically proceeding to Task Synthesis (Phase 3)...")
            task_synthesis_success = run_task_synthesis(folder_name)
...
"""
    ...
    parser.add_argument(
        "--skip-task-synthesis",
        action="store_true",
        help="Skip automatic task synthesis after exploration (Phase 3)"
    )
