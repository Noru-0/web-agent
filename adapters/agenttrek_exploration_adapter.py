...
class AgentTrekExplorationAdapter(ExplorationAdapter):
    ...
    def __init__(
        ...
        self.api_key = api_key or env.get("EXPLORER_API_KEY") or env.get("HUGGINGFACE_API_KEY")
        self.temperature = temperature or env.get_float("EXPLORER_TEMPERATURE", 0.2)
        
        # Validate configuration
        if not self.api_key:
            logger.warning(
                "No HuggingFace API key found. Set HUGGINGFACE_API_KEY or EXPLORER_API_KEY in .env"
            )
    ...
    def _init_client(self):
        ...
        try:
            ...
            logger.debug("OpenAI-compatible client initialized for HuggingFace")
    ...
    async def _execute_via_agenttrek(
        ...
        try:
            ...
            action = self._parse_grounding_response(llm_output, intent, dom_elements)
            if not action:
                logger.warning("Failed to parse AgentTrek grounding response")
                return False
        ...
        try:
            ...
            if action_type == ActionType.NAVIGATE or element_index == 0:
                ...
                if 0 <= actual_index < len(dom_elements):
                    target = dom_elements[actual_index].get('selector', '')
                    if not target:
